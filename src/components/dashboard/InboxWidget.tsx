...
const InboxWidget: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'all' | 'gmail' | 'slack' | 'teams'>('all');
    ...
    try {
      ...
      if (slackResponse.success && slackData?.messages) {
        ...
        const slackMessages: Message[] = rawMessages.map((msg: any) => ({
          ...
          subject: msg.channel ? `#${msg.channel}` : '#general', // Default to #general if not in msg
          preview: msg.text || '',
      ...
      setMessages(prev => {
        // Keep existing mocked 'teams' messages if meaningful, or just replace with fetched data
        // Let's filter out old Gmail/Slack mock data to be safe and keep others (e.g. Teams)
        const others = prev.filter(m => m.source === 'teams');
